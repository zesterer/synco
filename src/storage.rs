...
pub trait Storage<T: Component>: Resource + Default {
    ...
    unsafe fn get_unchecked(&self, entity: Entity) -> Self::Ref<'_>;
    /// Safety: The entity must have an already-inserted component in this storage.
    unsafe fn get_unchecked_mut(&mut self, entity: Entity) -> Self::RefMut<'_>;
    /// Safety: The entity must not have an already-inserted component in this storage.
    unsafe fn insert_unchecked(&mut self, entity: Entity, item: T);
    /// Safety: The entity must have an already-inserted component in this storage.
    unsafe fn remove_unchecked(&mut self, entity: Entity) -> T;
}
